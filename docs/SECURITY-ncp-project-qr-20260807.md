...
- Put/Delete는 **본문 SHA-256 서명**(CLI PutObject와 동일 계열). GET presign만 UNSIGNED-PAYLOAD 유지.
- 전송하는 `Content-Type`을 SignedHeaders에 포함.
- Vercel에서 헤더가 흔들리지 않도록 Put/Delete는 Node `https.request` 사용.
