...
- HWPX 보내기 — 앱이 사진·별도영역 표를 직접 만듦. 한글 서식 파일(`report.hwpx`)을 읽지 않음. 글꼴 파일은 넣지 않으며, 문서에는 OFL 글꼴 이름(Noto Sans KR)만 적고 끼워 넣지 않음. 저장소에 남아 있는 서식 파일은 현재 보내기에서 사용하지 않음. Copyright © 2026 이형우
...
| 2026-09-11 | HWPX 보내기는 서식 파일을 쓰지 않고 앱이 사진·별도영역 표를 만듦. 글꼴 파일 없음. 신규 의존성·GPL 없음 |
