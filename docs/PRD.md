...
| 문서 작성일 | 2026-07-31 |
| 문서 동기화 | 2026-09-15 — 글 칸 사진 위 표시 · 권장 `112033` |
| 최신 반영 커밋 | (본 배포) 큰 미리보기 글 칸이 사진 위에 보임 |
...
| US-83 | 새 저장 화면에서 **항목마다 이어서** 제목·장소·메모를 말하고 싶다 (설정 opt-in) | P2 | ✅ `8856461`·`6cd1dc4` |
| US-84 | 항목 말하기 중 **저장 유형을 바꾸고** 템플릿 **말하기 예**를 보고 싶다 | P2 | ✅ `09f9c87`·`6cd1dc4` |
| US-85 | 저장·수정 화면에서 미리보기와 같은 JPEG를 **바로 갤러리에** 넣고 싶다 | P2 | ✅ |
...
[카메라] 시스템 카메라 자동 실행(줌) · 촬영 전 **위치 워밍업** · 촬영 확인 시트 열림 중 **GPS 프리페치**(fast→정밀 백그라운드)
    ...
    → 미리보기 아래 [이미지 저장] → 앱 저장(수정) + 캡션 JPEG 갤러리(웹 다운로드). 목록 「이미지 저장」과 같은 합성
...
| F-Voice-10 | **저장 직후 음성으로 항목 채우기** — 설정 opt-in, 신규 저장 시 「항목 말하기」시트(제목→장소→메모; 추가1~3·QR 비포함) | ✅ `8856461` · `restore-save-slot-speech.bat` · 표시명 `restore-item-speak-label.bat` |
| F-Voice-11 | 항목 말하기 시트 **저장 유형 바꾸기** + 항목별 템플릿 **말하기 예** | ✅ `09f9c87` · `restore-slot-speech-type-hint.bat` |
...
| F-EXP-03 | 선택 스탬프 → **HWPX** (한글 저장 서식 `report.hwpx` 한 칸에 사진·앱 캡션 덩어리; 표가 한 쪽에 들어가도록 사진을 조금 작게; 첫 사진은 제목 다음 쪽; **바이너리 저장**) | ✅ |
...
| **F-QR-01** | ~~미구현~~ → ✅ caption MVP+마이크·`https://`·**연결확인**(`172149`) — 워터마크 코너·PDF/XLSX/HWPX QR는 이후 | 확장 후보 |
...
| **2026-09-17** | 내보내기 기본 파일명에 **장소** · 여러 장 **이미지 저장은 장별 이름** · 도움말 · APK `175121` · `restore-export-file-place.bat` | (본 커밋) |
| **2026-09-17** | 사진 위 글 **저장 위치를 글자 상단에 맞춤** · 도움말 · APK `102508` · `restore-note-pad-rise.bat` | (이전) |
| **2026-09-17** | 저장·수정 **칸 본문 글자색**을 흰 바탕에 어둡게 고정 · 도움말 · APK `094104` · `restore-field-ink.bat` | (이전) |
| **2026-09-16** | **글 넣기**(닫기 옆)·크기·투명도 막대·저장 위치 맞춤 · 도움말 · APK `104154` · `restore-note-pad-tune.bat` | (이전) |
...
| **2026-09-15** | 저장·수정 입력칸이 **키보드에 가리지 않게** · 도움말 · APK `075220` · `restore-save-kb-clear.bat` | (이전) |
| **2026-09-14** | 수신함 **이 폰** 사진 표시 · 도움말 · APK `230623` · `restore-inbox-on-device.bat` | (이전) |
| **2026-09-14** | 저장 목록 **사업으로 보내기** 고르기·연결 확인 · 도움말 · APK `223408` · `restore-join-list-send-check.bat` | (이전) |
| **2026-09-14** | 저장 화면 **시스템 뒤로가기**가 흰 화면에 머물지 않게 · 도움말 · APK `190436` · `restore-save-sheet-back.bat` | (이전) |
...
| **2026-08-23** | 연결 비교 엑셀 **사진 가로 px**·글자 크기 · 도움말 · APK `181951` · `restore-follow-xlsx-photo-px.bat` | `bba7014` |
...
> **2026-09-17:** 권장 파일은 **`VoiceStamp_20260917_175121.apk`**. 채널: [RELEASE-CHANNELS.md](./RELEASE-CHANNELS.md). 성능: [HEALTHCHECK.md](./HEALTHCHECK.md).
...
| `releases/VoiceStamp_20260802_124143.apk` | `2a00578` | **이전** — 선택 취소 후 목록 썸네일 유지 (+08-02 누적) |
...
| OCR→제목·메모 초안 설계 (AI-ML-03) | [DESIGN-ML-KIT-OCR-TITLE.md](./DESIGN-ML-KIT-OCR-TITLE.md) |
