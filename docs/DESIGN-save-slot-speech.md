...
5. 칸마다 템플릿 `placeholders`를 「말하기 예」로 표시(힌트만)
6. title → place → memo, 칸마다 `useSpeechInput.start()` (continuous: false)
7. 비어 있지 않은 칸만 부모 state에 반영 + touched 플래그
